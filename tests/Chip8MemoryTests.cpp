...
{
    chip8core::Chip8Memory mem;
    EXPECT_THROW(mem.read(chip8core::Chip8Memory::MEMORY_SIZE), chip8core::Chip8MemoryException);
}
...
{
    chip8core::Chip8Memory mem;
    EXPECT_THROW(mem.write(chip8core::Chip8Memory::MEMORY_SIZE, 0x12),
                 chip8core::Chip8MemoryException);
}
...
TEST(Chip8MemoryTest, ClearMemorySetsAllZero)
{
    ...
    for (uint16_t i = 0; i < chip8core::Chip8Memory::MEMORY_SIZE; ++i)
    {
        mem.write(i, 0xAB);
    }
...
TEST(Chip8MemoryTest, WriteBlockOutOfBoundsThrows)
{
    ...
    uint16_t address = chip8core::Chip8Memory::MEMORY_SIZE - 5;
    EXPECT_THROW(mem.write(address, data), chip8core::Chip8MemoryException);
}
...
TEST(Chip8MemoryTest, ReadBlockOutOfBoundsThrows)
{
    ...
    size_t                 length  = 10;
    uint16_t               address = chip8core::Chip8Memory::MEMORY_SIZE - 5;
    EXPECT_THROW(mem.read(address, length), chip8core::Chip8MemoryException);
}
...
TEST(Chip8MemoryTest, WriteAndReadBlockAtEnd)
{
    ...
    uint16_t               address = chip8core::Chip8Memory::MEMORY_SIZE - data.size();
    mem.write(address, data);
...
TEST(Chip8MemoryTest, DumpReturnsFullMemory)
{
    ...
    ASSERT_EQ(dumped.size(), chip8core::Chip8Memory::MEMORY_SIZE);
